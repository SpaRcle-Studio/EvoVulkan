...
namespace EvoVulkan::Types {
    ...
    static RenderPass CreateRenderPass(
        ...
        /// просто цвет без мультисемплинга
            ...
            auto&& dependencyColor = dependencies.emplace_back();

            dependencyColor.sType = VK_STRUCTURE_TYPE_SUBPASS_DEPENDENCY_2;
            dependencyColor.pNext = nullptr;
            dependencyColor.srcSubpass = VK_SUBPASS_EXTERNAL;
            dependencyColor.dstSubpass = 0;
            dependencyColor.srcStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;
            dependencyColor.dstStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;
            dependencyColor.srcAccessMask = 0;
            dependencyColor.dstAccessMask = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT;
            dependencyColor.dependencyFlags = VK_DEPENDENCY_BY_REGION_BIT;

            if (depth) {
                auto&& dependencyDepth = dependencies.emplace_back();
                dependencyDepth.sType = VK_STRUCTURE_TYPE_SUBPASS_DEPENDENCY_2;
                dependencyDepth.pNext = nullptr;
                dependencyDepth.srcSubpass = VK_SUBPASS_EXTERNAL;
                dependencyDepth.dstSubpass = 0;
                dependencyDepth.srcStageMask = VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT | VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT;
                dependencyDepth.dstStageMask = VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT | VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT;
                dependencyDepth.srcAccessMask = 0;
                dependencyDepth.dstAccessMask = VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT;
                dependencyDepth.dependencyFlags = VK_DEPENDENCY_BY_REGION_BIT;
            }

            auto& dependencyColorOutput = dependencies.emplace_back();
            dependencyColorOutput.sType = VK_STRUCTURE_TYPE_SUBPASS_DEPENDENCY_2;
            dependencyColorOutput.pNext = nullptr;
            dependencyColorOutput.srcSubpass = 0;
            dependencyColorOutput.dstSubpass = VK_SUBPASS_EXTERNAL;
            dependencyColorOutput.srcStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;
            dependencyColorOutput.dstStageMask = VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT;
            dependencyColorOutput.srcAccessMask = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT;
            dependencyColorOutput.dstAccessMask = VK_ACCESS_SHADER_READ_BIT;
            dependencyColorOutput.dependencyFlags = VK_DEPENDENCY_BY_REGION_BIT;

            if (depth) {
                auto& dependencyDepthOutput = dependencies.emplace_back();
                dependencyDepthOutput.sType = VK_STRUCTURE_TYPE_SUBPASS_DEPENDENCY_2;
                dependencyDepthOutput.pNext = nullptr;
                dependencyDepthOutput.srcSubpass = 0;
                dependencyDepthOutput.dstSubpass = VK_SUBPASS_EXTERNAL;
                dependencyDepthOutput.srcStageMask = VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT;
                dependencyDepthOutput.dstStageMask = VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT;
                dependencyDepthOutput.srcAccessMask = VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT;
                dependencyDepthOutput.dstAccessMask = VK_ACCESS_SHADER_READ_BIT;
                dependencyDepthOutput.dependencyFlags = VK_DEPENDENCY_BY_REGION_BIT;
            }
        }
        /// только буфер глубины без мультисемплинга
            ...
            dependencyDepth.dstStageMask = VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT | VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT;
            dependencyDepth.srcAccessMask = 0;
            dependencyDepth.dstAccessMask = VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT | VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT;
            dependencyDepth.dependencyFlags = VK_DEPENDENCY_BY_REGION_BIT;
            ...
            dependencyDepthOutput.srcStageMask = VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT;
            dependencyDepthOutput.dstStageMask = VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT;
            dependencyDepthOutput.srcAccessMask = VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT | VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT;
            dependencyDepthOutput.dstAccessMask = VK_ACCESS_SHADER_READ_BIT;
            dependencyDepthOutput.dependencyFlags = VK_DEPENDENCY_BY_REGION_BIT;
        }
